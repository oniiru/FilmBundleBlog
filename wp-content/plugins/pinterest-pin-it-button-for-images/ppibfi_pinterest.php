	...
	foreach( $images as $image ){
		...
		if( $any_image_has_the_needed_pin_class ){
			...
			if( pibfi_Engine_check_if_the_image_has_the_forbidden_class( $image['tag'] ) ){
				$needed2 = is_insta_link( $image[ 'tag' ] );
				
				if( $needed2 ) {
					$image_tag = sprintf( $replacementinsta, $image[1], $image[2], $image[3], $image[4]);
					$content = str_replace( $image['tag'], $image_tag, $content);
				}
				else {
...
function is_insta_link ( $tag ) {
	
		preg_match( '/class=[\'"](.*)?' . trim( 'instathis' ) . '(.*)?[\'"]/i', $tag, $matches );
		if ($matches) {
			return true;
		}
		return false;
